...
%      MOTIFEDITOR, by itself, creates a new MOTIFEDITOR or raises the existing
%      singleton*.
%
...
axis tight
set(gca,'ytick',loc);
set(gca,'yticklabel',num2strVec(1:t));
...
defaultans = {num2str(Motif(f).thres)};
answer = inputdlg(prompt,dlg_title,[1,50],defaultans);
thresh=str2num(answer{1});
if thresh>0&&thresh<1
    [start,stop,center,warp]=findMotifs([handles.F,handles.files{f}],template,thresh);
    Motif(f).start=start;
    Motif(f).stop=stop;
